...
*来源核验（对照参考论文官方仓库，杜绝任何来源混淆）。* 我们克隆了该论文的
官方配套仓库 `taojinllnl/Phasefield_L-BFGS_monolithic_solver`（论文全部算例
的输入文件）逐项核验：

| 项 | 官方仓库输入 | 本框架 | 一致 |
|---|---|---|---|
| SNT/SNS 材料 | `121.15 80.77 0.0075 2.7e-3`（λ, μ, l, gc） | 同 | ✓ |
| SNS 场景 | Scenario 4（结构化网格剪切）、AT2、平面应力、t→15e-3 | 同 | ✓ |
| TPB 材料/时间 | `12.0 8.0 0.0075 5.0e-4`、t→6.0e-2（×−1 向下） | 同 | ✓ |
| **SNT 参考锚点** | 仓库自带日志为 **PFCZM** 示例（峰值 0.4818 @ 4.5e-3） | 本文锚点为**论文发表 AT2 图表**（0.7565 @ 5.9e-3） | **明确区分** ✓ |

即：本文参考锚点既不是仓库默认示例（PFCZM）的运行数据，也不是任何网络
图片，而是论文正式发表图表（AT2 模型）的数字化结果；本框架的 AT2 复算
（0.7393 @ 6.01e-3，−2.3%）与论文 AT2 图吻合，而与 PFCZM 示例曲线
（无回跳、峰值 0.48）明显不同——模型辨识无误。
